...
// stats_daily er nøglet på YouTube-video-ID (11 tegn), ikke vores interne video_id.
// Udtræk YT-ID'et fra videos.urls.yt_url så opslag (titel m.m.) virker.
function ytIdOf(urls) {
  const u = urls && (urls.yt_url || urls.youtube || urls.yt);
  if (!u) return null;
  const m = String(u).match(/(?:youtu\.be\/|[?&]v=)([A-Za-z0-9_-]{11})/);
  return m ? m[1] : null;
}
...
export function computeWinnerLoop({ stats = [], videos = [] }) {
  ...
  videos.forEach((v) => {
    vmap[v.video_id] = v;
    const yid = ytIdOf(v.urls);
    if (yid && !vmap[yid]) vmap[yid] = v; // map også YT-ID → video (matcher stats_daily)
  });
