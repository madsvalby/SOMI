...
// Push ny rækkefølge til n8n (op=reorder) — opdaterer KUN priority pr. case_id
// (intake-webhooken rører ikke status). items = [{case_id, priority}]. MASTER
// henter 'ready' i priority ASC, så lav priority = produceres først.
export async function pushReorder(items) {
  const url = process.env.N8N_IDEAS_WEBHOOK_URL;
  if (!url) return { configured: false, ok: false };
  if (!items || !items.length) return { configured: true, ok: true, sent: 0 };
  try {
    const res = await fetch(url, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ op: "reorder", ideas: items.map((i) => ({ case_id: i.case_id, priority: i.priority })) }),
    });
    return { configured: true, ok: res.ok, status: res.status, sent: items.length };
  } catch (e) {
    return { configured: true, ok: false, error: String((e && e.message) || e) };
  }
}
